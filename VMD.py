...
=======
    ...
    # Extend signal by mirroring
#    T = save_t
    sig_mir = signal[:t_len/2+1][::-1]
    sig_mir = np.append(sig_mir,signal)
    sig_mir = np.append(sig_mir, signal[t_len/2+1:][::-1])
    ...
    # Spectral Domain discretization
    # TODO: Can this be replaced with fftfreq?
    freqs = (t-0.5)-(1/T)

>>>>>>> f8d7d8986b41c319f2ab1bd2d1b2e5f1312aa3d9
...
=======
    ...
    # Contruct and center sig_hat_plus
    sig_hat_plus = fftpack.fftshift(fftpack.fft(sig_mir))
    sig_hat_plus[:T/2] = 0
...
>>>>>>> f8d7d8986b41c319f2ab1bd2d1b2e5f1312aa3d9
    ...
    if init == 1:
        ...
        omega_plus[0:] = np.sort(np.exp(np.log(fs) + (
                              np.log(0.5) - np.log(fs)) * np.random.random(K)))
    else:
        omega_plus[0,:] = 0
    ...
    # Other inits
    uDiff = np.array([2 * tol]) #update step
    n = 0 # loop counter
...
=======
    while (uDiff.any() > tol) and (n < N - 1): # Not converged and below iterations limit
>>>>>>> f8d7d8986b41c319f2ab1bd2d1b2e5f1312aa3d9
        #update first mode accumulator
        sum_uk = u_hat_plus[n,:,-1] + sum_uk - u_hat_plus[n,:,0]

        # Update spectrum of first mode through Wiener filter of residuals
        u_hat_plus[n+1,:,0] = (sig_hat_plus - sum_uk - lambda_hat[n,:]/2) / \
                            (1 + Alpha[0,0] * (freqs - omega_plus[n,0])**2)
...
=======
            omega_plus[n,0] = np.dot(freqs[T/2+1:T],(np.abs(u_hat_plus[n, T/2+1:T, 0])**2).T) / \
                                 np.sum(np.abs(u_hat_plus[n,T/2+1:T,0])**2)
...
>>>>>>> f8d7d8986b41c319f2ab1bd2d1b2e5f1312aa3d9
            ...
            # Mode spectrum
            u_hat_plus[n, :, k] = (sig_hat_plus - sum_uk - lambda_hat[n,:]/2) /\
        ...
        # Dual ascent
        lambda_hat[n,:] = lambda_hat[n,:] + tau * (np.sum(u_hat_plus[n,:,:],axis=1) - sig_hat_plus)
